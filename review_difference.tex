...
%DIF ADD holofood_new.tex   Mon Jul 14 18:18:54 2025
...
\\ % New line
\textbf{Supplementary information:} Available in the package vignette \url{https://ebi-metagenomics.github.io/HoloFoodR/articles/case_study.html}
}
\DIFaddend %\textbf{Motivation:} Holo-omics is an emerging research area that integrates multi-omic data from the host organism and its microbiome to study their interactions. Recently, curated and openly accessible holo-omic databases have been developed. The HoloFood database, for instance, provides nearly 10,000 holo-omic profiles for salmon and chicken under controlled treatments. Despite the improvements in data availability, bridging the gap between holo-omic data resources and algorithmic frameworks remains a challenge. Method development in this area can facilitate the sharing of methods and best practices in the field of holo-omics.
%\\ % New line
...
Data containers provide structured, standardized storage for data, proving useful in life science informatics where complex, hierarchical and multi-source data collections are common to describe the studied phenomenon (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
). Bioconductor's }\DIFaddend \texttt{mia} framework \DIFdelbegin \DIFdel{centering }\DIFdelend \DIFaddbegin \DIFadd{centers }\DIFaddend around these containers and supports a range of analysis and visualization methods \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend microbiome research (\cite{mia, miaViz, oma}). Similarly, the metabolomic data can be pre-processed, analyzed and visualized with the \texttt{notame} package (\cite{klavus_2020}).
...
\DIFdelbegin \DIFdel{The HoloFoodR software library }\DIFdelend \DIFaddbegin \DIFadd{HoloFoodR }\DIFaddend simplifies data retrieval by minimizing external data wrangling \DIFaddbegin \DIFadd{and }\DIFaddend adhering to standardized data structures that support efficient use of the multi-omic analyses methods. This standardization replaces lengthy \textit{ad-hoc} code with concise \DIFdelbegin \DIFdel{, }\DIFdelend and tested open-source solutions. We \DIFdelbegin \DIFdel{illustrate the }\DIFdelend \DIFaddbegin \DIFadd{demonstrate this }\DIFaddend approach with an end-to-end workflow\DIFdelbegin \DIFdel{that covers typical steps of a holo-omic data analysis and emphasizes }\DIFdelend \DIFaddbegin \DIFadd{, highlighting }\DIFaddend the use of multi-assay data containers to enhance access to \DIFdelbegin \DIFdel{methods in the Bioconductor ecosystem}\DIFdelend \DIFaddbegin \DIFadd{Bioconductor methods}\DIFaddend .

% Use in teaching
\DIFdelbegin \DIFdel{In addition to }\DIFdelend \DIFaddbegin \DIFadd{Beyond }\DIFaddend method development and analysis, \DIFdelbegin \DIFdel{we see potential in using HoloFood data }\DIFdelend \DIFaddbegin \DIFadd{HoloFood data can aid }\DIFaddend in teaching multi-omics techniques. HoloFoodR offers simplified access to real-world omic data, enabling more proficient users \DIFaddbegin \DIFadd{to }\DIFaddend "learn by doing”. The \DIFdelbegin \DIFdel{advantage of such data over }\DIFdelend \DIFaddbegin \DIFadd{complexity of this data makes it superior to }\DIFaddend “toy” \DIFdelbegin \DIFdel{data lies in its increased complexity, which provides an excellent foundation for instructors to teach students }\DIFdelend \DIFaddbegin \DIFadd{datasets for teaching }\DIFaddend advanced data cleaning and analysis \DIFdelbegin \DIFdel{techniques (\mbox{%DIFAUXCMD
\cite{drnevich_2024}}\hskip0pt%DIFAUXCMD
